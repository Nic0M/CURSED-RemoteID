...
while true:
    if(glob.glob('D:\\Engineering\\Senior_Projects_Example_Data\\full_packet_capture_*.pcapng')!=[]):
        ...
        with open(csvPath, 'w', newline='',encoding="utf-8") as file:
            ...
            field = ["Source Address","Unique ID","Timestamp","Heading", "Ground Speed", "Vertical Speed","Latitude","Longitude","Barometric Altitude","Geodetic Altitude","Height","Horizontal Accuracy","Geodetic Vertical Accuracy","Barometric Altitude Accuracy","Speed Accuracy"]
            ...
            for pkt in pcap:
                ...
                try:
                    ...
                    Minute=math.floor(int(pkt['OpenDroneID'].get_field('OpenDroneID.loc_timeStamp'))/10/60)
                    Second=math.floor(int(pkt['OpenDroneID'].get_field('OpenDroneID.loc_timeStamp'))/10-(Minute*60))
                    TimeStamp= Year+'-'+Month+'-'+Day+' '+B+':'+str(Minute)+':'+str(Second)
                    ...
                    pressALT=pkt['OpenDroneID'].get_field('OpenDroneID.loc_pressAlt')
                    geoALT=pkt['OpenDroneID'].get_field('OpenDroneID.loc_geoAlt')
                    height=pkt['OpenDroneID'].get_field('OpenDroneID.loc_height')
                    Haccuracy=pkt['OpenDroneID'].get_field('OpenDroneID.loc_hAccuracy')
                    Vaccuracy=pkt['OpenDroneID'].get_field('OpenDroneID.loc_vAccuracy')
                    Baroaccuracy=pkt['OpenDroneID'].get_field('OpenDroneID.loc_baroccuracy')
                    Speedaccuracy=pkt['OpenDroneID'].get_field('OpenDroneID.loc_speedAccuracy')
                except TypeError:
                    pass
                if (Minute < 60):
                    if str(ID).isascii():
                        writer.writerow([access_address,ID,TimeStamp,direction,speed,vspeed,lat,lon,pressALT,geoALT,height,Haccuracy,Vaccuracy,Baroaccuracy,Speedaccuracy])
                    else:
                        continue
        pcap.close()
        os.remove(path)
